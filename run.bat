...
:build_share
    ...
    docker-compose -f "%COMPOSE_FILE_PATH%" rm -f katco-share
	call %MVN_EXEC% clean package -pl katco-share,katco-share-docker
EXIT /B 0
:build_acs
    docker-compose -f "%COMPOSE_FILE_PATH%" kill katco-acs
    docker-compose -f "%COMPOSE_FILE_PATH%" rm -f katco-acs
	call %MVN_EXEC% clean package -pl katco-integration-tests,katco-platform,katco-platform-docker
EXIT /B 0
:tail
...
EXIT /B 0
:prepare-test
    call %MVN_EXEC% verify -DskipTests=true -pl katco-platform,katco-integration-tests,katco-platform-docker
EXIT /B 0
:test
    call %MVN_EXEC% verify -pl katco-platform,katco-integration-tests
EXIT /B 0
:purge
